...
(function (global, factory) {
  ...
  /* Global */ else { (global['dcodeIO'] = global['dcodeIO'] || {})['WebRcon'] = factory() }
  ...
  WebRcon.prototype._connect = function (endpoint) {
    if (!this.closed) { throw Error('already connected') }
  ...
  WebRcon.prototype.run = function run (command, identity) {
    return new Promise((resolve, reject) => {
      if (identity != null) { listeners[identifier] = identity }

      identifier += 1
      if (identifier >= 2000) { identifier = 1001 }
  ...
  WebRcon.prototype.removeListener = function removeListener (event, callback) {
    ...
    if (index < 0) { return }
    if (listeners.length === 1) { delete this._listeners[event] } else { listeners.splice(index, 1) }
  }
  ...
  WebRcon.prototype.emit = function emit (event, a1, a2, a3) {
    var listeners = this._listeners[event],
      k
    if (!(listeners && (k = listeners.length))) { return }
    for (var i = 0; i < k; ++i) { listeners[i](a1, a2, a3) }
  }
